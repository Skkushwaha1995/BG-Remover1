...
def process_images(files, base, size, replace_bg, bg_color):
    ...
    for idx, f in enumerate(files, 1):
        ...
        buf = BytesIO(); img.save(buf, "PNG", optimize=True, quality=85); buf.seek(0)
...
uploaded = st.file_uploader("", type=["png","jpg","jpeg","bmp"],
    ...
    for i, buf in enumerate(st.session_state["raw_urls"]):
        with cols[i%4]:
            st.image(buf, use_container_width=True,
                     caption=getattr(buf, "name", f"URL_{i+1}"))
...
if st.session_state.get("proc_urls"):
    ...
    for i, info in enumerate(st.session_state["proc_urls"]):
        with cols[i%4]:
            ...
            st.download_button("⬇️ Download", data=info["buf"],
                               file_name=info["name"], mime="image/png",
                               key=f"dl_url_{i}")
    # ZIP
    zipb=BytesIO()
    ...
    zipb.seek(0)
    st.download_button("📦 Download All URLs ZIP", data=zipb,
                       file_name="urls_processed.zip", mime="application/zip")
...
if st.session_state.get("proc_files"):
    ...
    with zipfile.ZipFile(zipb,"w") as zf:
        for info in st.session_state["proc_files"]:
            zf.writestr(info["name"], info["buf"].getvalue())
    zipb.seek(0)
    st.download_button("📦 Download All Local ZIP", data=zipb,
                       file_name="files_processed.zip", mime="application/zip")
